...
def create_markdown(title: str, content: str, image_paths: List[str], output_path: str):
    ...
    Raises:
        ValueError: 标题、内容和图片不能同时为空
        IOError: 文件写入失败
    """
    # 输入验证
    if not title and not content and not image_paths:
        raise ValueError("标题、内容和图片不能同时为空")
    ...
    # 标题
    display_title = title or "小红书笔记"
    markdown_lines.append(f"# {display_title}\n")
    ...
    if image_paths:
        ...
        for i, img_path in enumerate(image_paths, 1):
            try:
                ...
                logger.error(f"处理图片路径失败 {img_path}: {e}")
...
def create_zip(markdown_path: str, image_dir: str, output_path: str):
    ...
    """
    # 输入验证
    if not markdown_path or not isinstance(markdown_path, str):
        raise ValueError("markdown_path 必须是非空字符串")

    if not image_dir or not isinstance(image_dir, str):
        raise ValueError("image_dir 必须是非空字符串")

    if not output_path or not isinstance(output_path, str):
        raise ValueError("output_path 必须是非空字符串")
    ...
    try:
        ...
        logger.error(f"ZIP 文件创建失败: {e}")
        raise IOError(f"ZIP 文件创建失败: {e}")
    except IOError as e:
        logger.error(f"文件操作失败: {e}")
        raise
    except Exception as e:
        logger.error(f"创建 ZIP 失败: {e}")
        raise IOError(f"创建 ZIP 失败: {e}")
